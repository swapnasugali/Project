...
const allPresentButton = document.getElementById("allPresentButton");
...
function showStudents() {
  ...
  if (students.length === 0) {
    studentTable.innerHTML = `
      <tr>
        <td colspan="9">No students found. Add students from Student Registration first.</td>
      </tr>
    `;
  ...
  students.forEach(function (student, index) {
    ...
    row.innerHTML = `
      ...
      <td class="present">
        <input type="radio" name="attendance-${studentId}" value="Present"
          ${status === "Present" ? "checked" : ""}>
        Present
      </td>

      <td class="absent">
        <input type="radio" name="attendance-${studentId}" value="Absent"
      ...
      <td>
        <button type="button" class="history-button">View History</button>
      </td>
    `;
...
allPresentButton.addEventListener("click", markAllPresent);
