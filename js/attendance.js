...
function getPercentage(studentId, monthly) {
    ...
    if(monthly){
        ...
        let month = dateInput.value.slice(0,7);
    ...
    let present = studentRecords.filter(function(record){
...
function updateCards(){
    ...
    let todayRecords = records.filter(function(record){

        return record.date==today;

    });

    let present = todayRecords.filter(function(record){

        return record.status=="Present";

    }).length;

    let absent = todayRecords.filter(function(record){
    ...
    totalStudentsText.textContent = students.length;
    presentStudentsText.textContent = present;
    absentStudentsText.textContent = absent;
...
function showStudents(){
    ...
    students.forEach(function(student,index){
        ...
        let oldRecord = records.find(function(record){
        ...
        let status = oldRecord ? oldRecord.status : "Present";
        ...
        <td>${getPercentage(student.studentId,false)}%</td>

        <td>${getPercentage(student.studentId,true)}%</td>

        <td>

        <button class="history-button"

        onclick="viewHistory('${student.studentId}','${student.fullName}')">
...
function allPresent(){
    ...
    students.forEach(function(student){
        ...
        `input[name="attendance-${student.studentId}"][value="Present"]`
...
function saveAttendance(){
    ...
    students.forEach(function(student){
        ...
        `input[name="attendance-${student.studentId}"]:checked`
        ...
        let index = records.findIndex(function(record){
        ...
        if(index==-1){
            ...
            records[index].status=radio.value;
...
function viewHistory(studentId,studentName){
    ...
    let history = records.filter(function(record){

        return record.studentId==studentId;

    });

    let present = history.filter(function(record){

        return record.status=="Present";

    }).length;

    let absent = history.filter(function(record){

        return record.status=="Absent";

    }).length;

    let percentage = history.length ?

    Math.round((present/history.length)*100):0;
    ...
    if(history.length==0){
        ...
        historyData.innerHTML="<p>No Records Found</p>";

    }

    else{

        historyData.innerHTML="<ul>";
...
dateInput.addEventListener("change",showStudents);
